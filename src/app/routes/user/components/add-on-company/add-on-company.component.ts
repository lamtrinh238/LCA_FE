...
export class AddOnCompanyComponent implements OnInit {
    ...
    constructor(private clientService: ClientService, private formBuider: FormBuilder) {
    ...
    loadCompanyFilter(): void {
        const listCompany$: Observable<CompanyModel[]> = this.searchChange$.asObservable().pipe(
            debounceTime(500),
            tap((res) => {
                if (res != '') {
