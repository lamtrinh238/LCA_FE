...
export class QueryParamObject {
  ...
  toApiFilterQuery(): {
    ...
    const sortApi =
      this.sort && this.sort.filter((s: SortObject) => s.value !== undefined && s.value !== null);
    const filterApi =
      this.filter &&
      this.filter.filter((f: FilterObject) => f.value !== null && f.value !== undefined);
    ...
    if (filterApi && filterApi.length > 0) {
      ...
      if (filterApiGlobal.length > 0) {
        filter.GlobalSearch =
          filterApiGlobal.map((sort: FilterObject) => sort.toFilterString()).join(',') +
          `[like]${filterApiGlobal[0].value}`;
      }
...
export abstract class BaseObjectFilterModel {
  ...
  toFilterObjects(): FilterObject[] {
    return this.filterBy.map(
      (by: string) => new FilterObject(by, this.filterTerm === '' ? null : this.filterTerm, 'like'),
    );
  }
  clear(): void {
